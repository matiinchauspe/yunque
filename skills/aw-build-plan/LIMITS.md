...
expand–contract batches `aw-slice-plan` sanctions for a wide refactor: they promise green only at a
downstream integrate-and-verify ticket, so each one is red by design.

**Only a named branch counts.** `aw-slice-plan` writes that line on batches alone and keeps it out
...
(`plan-prompt.md:25`) — which trades against `aw-spawn-worktree` refusing to reuse a name, a real
