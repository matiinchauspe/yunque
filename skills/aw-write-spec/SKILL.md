...
conversation and in the decisions `aw-grill-plan` persisted — gets consolidated into one
...
- **Ripeness.** Are the decisions settled? `recall` prior decisions on the task's keywords
  ...
  conversation is not **ripe** — surface those decisions and hand back to `aw-grill-plan`
  ...
  bounce a task back to `aw-grill-plan` on missing memory.
...
and they are the anchor `aw-slice-plan`'s tracer bullets verify against. Done when every
...
convention decides where it lands and under which key. `aw-slice-plan` will `fetch` it from
...
names a tracker; the spec publishes as the `spec` artifact kind alongside `aw-slice-plan`'s
