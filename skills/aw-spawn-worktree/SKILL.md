...
(`git worktree remove`, or a future `aw-clean-worktree`). This is the light isolation rung —
...
- `repos/<repo>/` exists and is a FULL clone — worktrees need full history, so a shallow
  clone won't do (`aw-sync-repo` clones full by default).
